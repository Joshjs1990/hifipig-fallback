...
(function () {
  ...
  /* -----------------------------
   * Dynamic sidebar ads
   * - Replaces sidebar AdRotate widgets with AJAX-loaded slots
   * - Adds one slot at a time until the sidebar reaches the content height
   * ----------------------------- */
  whenIdle(function initDynamicSidebarAds() {
    const sidebar = document.querySelector(".site-sidebar[data-dynamic-sidebar='true']");
    const slotsWrap = sidebar?.querySelector("[data-dynamic-sidebar-slots]");
    const contentArea = document.querySelector(".layout[data-sidebar] > .content-area");
    if (!sidebar || !slotsWrap || !contentArea) return;

    let rafId = 0;
    let timeoutId = 0;
    let loading = false;
    let stopped = false;
    const maxSlots = Number.parseInt(slotsWrap.getAttribute("data-slot-capacity") || "0", 10) || 0;
    let nextIndex = Number.parseInt(slotsWrap.getAttribute("data-next-index") || "0", 10) || 0;

    const getContentHeight = () =>
      Math.max(contentArea.scrollHeight, Math.ceil(contentArea.getBoundingClientRect().height));

    const getSidebarHeight = () =>
      Math.max(sidebar.scrollHeight, Math.ceil(sidebar.getBoundingClientRect().height));

    const targetReached = () => {
      const targetHeight = getContentHeight();
      const tolerance = 24;
      return !!targetHeight && getSidebarHeight() >= targetHeight - tolerance;
    };

    const bindSlotMedia = (slot) => {
      slot.querySelectorAll("img, iframe").forEach((el) => {
        el.addEventListener("load", schedule, { passive: true });
      });
    };

    const maybeLoadNext = async () => {
      if (loading || stopped || targetReached() || nextIndex >= maxSlots) {
        return;
      }
      if (!window.HiFiPig || !HiFiPig.ajaxUrl) {
        stopped = true;
        return;
      }
      ...
      try {
        const url = `${HiFiPig.ajaxUrl}?action=hifipig_sidebar_ad_slot&index=${encodeURIComponent(nextIndex)}`;
        const res = await fetch(url, { credentials: "same-origin" });
        const json = await res.json();
        const html = json?.data?.html || "";

        nextIndex += 1;
        slotsWrap.setAttribute("data-next-index", String(nextIndex));
        ...
        slotsWrap.classList.add("is-active");
        const tmp = document.createElement("div");
        tmp.innerHTML = html;
        const slot = tmp.firstElementChild;
        if (!slot) {
          return;
        }
      ...
      if (!targetReached() && nextIndex < maxSlots) {
        schedule();
      }
    };
    ...
    const update = () => {
      if (targetReached() || nextIndex >= maxSlots) {
        if (!slotsWrap.children.length) {
          slotsWrap.classList.remove("is-active");
        }
        return;
      }
    ...
    const schedule = () => {
      ...
      timeoutId = window.setTimeout(update, 180);
    };
